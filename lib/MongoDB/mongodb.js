...
const { MongoClient, ObjectId } = require('mongodb'); // Correctly import ObjectId from mongodb
...
// MongoDB connection URL and collection details
...
// Function to update the form status (e.g., profile, DL, etc.)
...
// Add user field (update user data)
...
// Upload file for user with a specific title
// Upload file for user with a specific title
async function uploadFileForUser(phoneNumber, fileData, fileName, uploadTitle) {
  try {
    ...
    // Update the user's document to link this file
...
// Delete file from GridFS and unlink from user
...
// Route for file upload using multer
// Route for file upload using multer
app.post('/uploadFileForUser', upload.single('file'), async (req, res) => {
  try {
    ...
    console.log("Received phoneNumber:", req.body.phoneNumber); // Logging phone number
    console.log("Received uploadTitle:", req.body.uploadTitle); // Logging upload title

    const phoneNumber = req.body.phoneNumber; // Phone number from form data
    const fileData = req.file.buffer; // The file's buffer
    const fileName = req.file.originalname;
    const uploadTitle = req.body.uploadTitle; // The upload title
    ...
    // Call the function to upload the file for the user
